...
def parse_template(template_docx: Path, bundle: StandardBundle) -> StageResult:
    ...
    if is_real_core_bundle(bundle):
        ...
        if baseline is not None and not baseline_findings:
            ...
            real_core_units = template_units.parse_reviewed_template_units(review_text)
            real_core_instruction_paragraphs = template_units.classify_instruction_paragraphs(
                paragraphs
            )
            template_units.apply_instruction_policy(
                paragraphs,
                real_core_instruction_paragraphs,
            )
            template_units.extend_slots_and_regions_from_units(
                slots,
                regions,
                real_core_units,
            )
    ...
    artifact = {
        ...
        "data": {
            ...
            "regions": regions,
            "slots": slots,
            "protected_zones": template_units.protected_zones_from_units(
                real_core_units
            ),
            "numbering": [],
            "headers_footers": [],
            "required_fields": template_units.required_fields_from_units(
                real_core_units
            ),
            "unsupported": unsupported,
        },
