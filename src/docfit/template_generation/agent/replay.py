...
def _legacy_transcript_submissions(
    transcript: dict[str, Any],
    *,
    max_rounds: int,
) -> list[dict[str, Any]]:
    ...
    for round_index, round_item in enumerate(transcript.get("rounds", []), start=1):
        ...
        if isinstance(round_submissions, list):
            submissions.extend(
                item for item in round_submissions if isinstance(item, dict)
            )
            continue
        submission = round_item.get("submission")
        if isinstance(submission, dict):
    ...
    if not submissions:
        for index, submission in enumerate(transcript.get("submissions", []), start=1):
            ...
            if isinstance(submission, dict):
                submissions.append(submission)
    return submissions[:max_rounds]


class ReplayTransport:
    provider = "replay"

    def __init__(self, transcript: dict[str, Any], *, max_rounds: int) -> None:
        self._submissions = transcript_submissions(transcript, max_rounds=max_rounds)
        self._offset = 0
    ...
    def complete_round(
        ...
        *,
        ...
        del messages, tools, response_format, max_tokens, temperature, tool_executor
        if self._offset >= len(self._submissions):
            return None
        submission = self._submissions[self._offset]
        self._offset += 1
        return submission
