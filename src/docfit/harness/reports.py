...
def write_report_bundle(
    ...
    *,
    ...
    summary = {
        ...
        "stage": stage,
        "status": status.value,
        "run_status": (run_status or status).value,
        "quality_status": (quality_status or status).value,
...
def build_pm_report(summary: dict[str, Any], findings: list[dict[str, Any]]) -> str:
    lines = [
        ...
        f"- Run status: {summary.get('run_status', summary['status'])}",
        f"- Quality status: {summary.get('quality_status', summary['status'])}",
