...
| `targets/<target>/v1/template_generation/*.expected.yaml` | 01-05 模板生成阶段的已签收期望 | 不替代阶段 verifier |
...
- `target.standard.yaml` 可以登记 `evidence_baselines.template_generation_stages` 字段；这个字段名表示“模板生成阶段基线清单”，不是旧目录名。
