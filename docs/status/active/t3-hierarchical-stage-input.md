...
层级 Stage Input、稀疏递归、atomic coverage、三路原子对账和 accepted mutation 精确消费已经进入正式 live/replay/template-generate 路径；另已增加从同一 gold-simulated T2 出发且关闭 Code/AI merge 的 Code 与 AI-primary 独立 Word 路线。结构与安全能力已实施，但三校动作准确率和调用成本尚未闭环，因此本状态项不能进入 verified/closed。

## Expected vs observed

Expected：T3 从 unit/object 开始，在最粗安全层级停止；只有混合对象沿合法直接 child 下钻；code/AI/merged 在共同 atomic identity 上比较，失败不进入 merged。

Observed：三校树和 coverage 已可闭合，run 已确定性展开为带精确字符范围的预生成 span 原子叶；完整同质 span 可安全投影回 raw run，混合 span 会显式冲突并阻止 merge。但 L1 仍缺 merge/nested/empty-cell 完整事实，TOC field 与展开段落缺所有权关系，live 会在目录过度下钻。
...
- template-generate replay contract 证明有序层级输入、sparse trace、atomic comparison 三份产物同 tree hash，且 coverage member 唯一。
...
- 下游：merged element_spec、T5/T6 actions 和 trace；fallback/contested 不得执行。
- 文档：Accuracy Promotion Gate 未通过前不更新 `docs/current/` canonical 架构和测试契约。
