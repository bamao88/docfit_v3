...
- Accuracy blocker: 已修复 gold evaluator 从旧 `page_text_index.style_details.runs` 读取造成的假 `raw_run_facts_missing`；重新生成的湖南农业大学 L1 现可通过原子事实门禁。当前本机缺少 `MINIMAX_API_KEY`，live 调用在模型启动前停止，因此尚无可信的前后 `exact_action_accuracy` 数字。
- Next proof: 配置同一 MiniMax 凭证后，在相同湖南农业大学 L1、T2 gold、模型和参数下分别运行 baseline commit 与当前实现。
