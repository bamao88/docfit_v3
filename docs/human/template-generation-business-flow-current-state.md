...
`/tmp/docfit_stage_realcore_audit29`，仍是 `FAIL`：湖南农业大学
`PASS/FAIL/UNKNOWN = 161/6/127`，南农 `94/20/92`，北大 `106/15/32`。
这说明当前生成器已经能产出可检查 Word 和明确证据，但还没有达到学校模板质量。
本轮改善集中在源锚点和检查定位：南农正文不再跳到最后的占位章标题；北大图目录、
表目录、正文、参考文献和致谢能定位到实际标题或正文开头；北大目录现在报告为
“生成标记已找到但顺序/分页不对”，不再误当图目录或表目录。
...
湖南和南农的大部分分页/分节失败收敛；北大剩余的分页失败主要来自目录标记顺序和
生成模板里的 section/header/footer/page-number 证据不足，还需要继续改生成顺序和
页眉页脚/页码生成。
