...
目的是让生成器按真实学校验收口径暴露差距。最新三校 probe 写在
`/tmp/docfit_stage_realcore_audit35`，仍是 `FAIL`：湖南农业大学
`PASS/FAIL/UNKNOWN = 176/7/111`，南农 `105/20/77`，北大 `109/13/28`。
这说明当前生成器已经能产出可检查 Word 和明确证据，但还没有达到学校模板质量。
本轮改善集中在源锚点和检查定位：南农正文不再跳到最后的占位章标题；北大图目录、
表目录、正文、参考文献和致谢能定位到实际标题或正文开头；北大源模板没有真实目录
标题时，生成器会在图目录前合成可见“目录”标题并设置分页，不再误把图目录或表目录
当目录。生成模板解析器现在也能读取 `word/footnotes.xml`，北大脚注不再作为未建模
可见对象阻断；文本框和图片也会进入 `generated_template_tree.json`，三校不再有
未建模可见对象阻断。页面设置检查现在会读取 section 的纸张、页边距、装订线和
页眉页脚距离，所以“纸张 A4”“常规页边距”“装订线”等规则能给出明确的 Word 证据。

如果当前阶段按“60 分内容草稿”验收，基础链路已经能跑通到 Word 输出：
bootstrap e2e 为 `PASS`；湖南农业大学 + `real-student-003` 会生成有效
`final.docx`，学生题名、摘要、关键词、正文、参考文献和致谢都在输出里，内部
`[[DOCFIT_*]]` 标记已清零。该真实样例仍按完整 gate 返回 `FAIL`，因为模板差距、
内容语义、放置位置、追加式渲染和 Word 图片证据还没达到完整验收。
...
当前生成器也已经开始写入真实 Word 分页边界：签名标准要求单元另起页时，会在
单元锚点上设置 `pageBreakBefore`；要求分节隔离时，会在单元前写入 `nextPage`
section break。生成过程记录会列出 `page_breaks` 和 `section_breaks`。这已经让
湖南和南农的大部分分页/分节失败收敛；北大目录分页已经能由合成标题证明。剩余失败
主要来自生成模板里的 section/header/footer/page-number 证据不足，还需要继续改
页眉页脚/页码生成。
