...
# T2/T3 Agent 方案 Plan 01：当前代码改进 + AI Overlay
...
round-0 deterministic T2/T3
  → optional render packet
  → replay/fixture/live transport 得到 layered AI response
  → projection：layered response → overlay proposals
  → thin reconciler：硬校验 only
  → T2/T3 overlay patch structure_candidates；T4 hints 落 artifact
  → regenerate unit_map / generation_model / element_spec
  → verify_template_parse_build 仍为唯一 status 权威
  → attribution：round-0 vs post-agent diff + proposal_id 映射
```
...
### 5.2 输出：layered response

AI 输出为 `template_agent_layered_response.json`：
...
```text
T2/T3:
  可投影为 overlay proposals 并 patch structure_candidates。

T4:
  首轮只写 agent_t4_hints.json；
  不 patch global_spec/template_spec/plan。
```

## 6. Reconciler 契约

探索期 reconciler 不判断 AI 语义是否正确，只防止不可执行输出进入主链。
...
template_agent_layered_response.json
...
Phase 2 schema + replay/fixture
  - render packet schema；
  - layered response schema；
  - ReplayTemplateAgentTransport；
  - FixtureTemplateAgentTransport；
  - 不接 runner、不接 live。

Phase 3 projection + reconciler + overlay
  - layered response → overlay proposals；
...
2. 零网络：replay/fixture 全链路可测，CI 不调用 live API。
...
8. 归因：同一模板可对比 agent off/on，并把变化映射到 applied_proposal_ids。
9. 非回归：standards/targets/** 不被自动更新；t2_standard gate 接线不被本路径触碰。
```
...
Q2 首轮一次输出 layers{t2,t3,t4_hints}，还是 T2/T3 分轮 focused-pass。
Q3 T3 source_seq/render target → stable_id 的多候选处理：rejected 还是 open_question。
Q4 T2 label suggestion 如何落 canonical/custom trace。
Q5 attribution 与 verification_report / template_gap_report 的最终字段合流方式。
Q6 产品化阶段是否恢复语义/置信 gate。
```
