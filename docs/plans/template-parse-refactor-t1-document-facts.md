...
- **段落节点**：`{text, style, raw_run_ids, logical_run_ids, structural_signals{…}}` + 分节/分页/编号/字段/表/`unknown_objects`。

**"原子"指什么（澄清用词，避免误读）**：指 T1 产出的判断是**单一、不可再合成的信号**（如 `is_toc_entry` 是一个布尔），**不是**把文字拆成最小颗粒。**T1 不切词**——"标题"是**一个 logical run**，不会变成"标"+"题"两个东西；即便像"湖 南 农 业 大 学"字间带空格，也是整条一个 run。

**责任边界（当前有越权，需归位）**：
- ⚠️ **`likely_unit_heading` 是 T1 越权**：它现在在 T1 里算了一个"这段是不是标题"的合成语义猜测，实现上其实是关键词匹配（`_looks_like_heading` 调 `_unit_for_text`）。"是不是边界/标题"属于 **T2 的合成决策职责**，T1 不该做。本轮已降级为 advisory（见 §2.3），最终应从 T1 输出移除。
...
- `is_spacing_line`：纯"空 N 行/格"括号（`（空三行）`、`（五号字空一行）`、`（根据题目长短四号字空二或三行）`）/ 空段。格式说明（`（一号黑体加粗）`）**不命中**，留给 `looks_like_instruction_text`。
...
### 2.3 `likely_unit_heading` 退役为 advisory（已处理，最终移除）【✅ 已完成 / 设计】
全仓库 grep 确认**无任何消费者**；本轮已加注释标为 advisory、明确"不作边界输入"。**最终建议**：T2 边界检测器落地后，从 `_structural_signals` 输出里删除该字段，把"是不是标题"的判断完全交给 T2（见 T2 文档 §2.1）。
