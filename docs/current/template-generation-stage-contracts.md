...
| 文档 | 负责回答 |
| --- | --- |
| `docs/current/template-generation.md` | 当前产品入口、真实运行流程、主要产物和使用方式是什么？ |
| 本文 | 每个阶段的长期职责、输入输出、身份、不变量和 route 契约是什么？ |
| `docs/current/template-generation-evaluation.md` | 每个阶段如何验收，哪些标准和 gate 能证明质量？ |
| `STATUS.md` / `full_summary.json` | 当前实现到哪里，现在哪个阶段阻塞？ |
| `docs/plans/**` | 从当前实现迁移到契约状态时，本轮具体改什么、删什么、怎么验证？ |

本文是规范性设计契约。2026-07-11，Plan 08 所覆盖的 L1 前置封存、T2-T7 shared hash/identity、旧输入退出和三校迁移等价条款已经验证；T3 run/span/policy/AI-primary 的长期目标仍由 T3 Plan 06 追踪。其他未落地条款必须在对应 plan、status 或 `full_summary` 中明确记录，不能把目标设计描述成当前能力。
...
- `unit_id`、单元边界或目录条目判断；
- `policy`、`role`、`fill_source`、`manual_only`、`generated`；
...
T2/T3/T4 的 code 和 AI 路线都只能通过 L1 或由 L1 构造的阶段输入读取客观事实。阶段实现不得直接读取：
...
| 使用类型 | 阶段 | 允许用途 |
| --- | --- | --- |
| 语义判断 | T2/T3/T4 code 与 AI | 从 L1 或 L1 派生的 Stage Input 识别单元、元素策略和全局版式 |
...
| T3 | L1 | 对应 route 的 T2 | `element_spec` | T5、T6、verifier |
...
| 页面引用 | 页号、图片 hash、宽高和类型 | 保留稳定取证引用；T2 文本模型当前不接收图片字节 |

预处理遵循以下顺序：

1. 从 sealed L1 白名单投影，保留原始 `source_seq`、文本事实、flow 类型和 render binding。
2. 删除重复或低判别信息，例如 `char_count`、默认 `false` 值、完整 bbox 和本机图片路径。
...
T3 在 T2 单元内，把 L1 的 source/run 事实整理为可执行 span 和元素，并判断每个元素是固定内容、模板默认内容、填写内容、人工内容、系统生成内容还是应删除说明。
...
- 缺少证据时允许 abstain/unknown，不能为了覆盖率硬认领。
...
| T3 route | 必须使用的 T2 输入 |
| --- | --- |
| `code_raw` | `T2.code_raw` |
| `ai_raw` | `T2.ai_raw` |
| `merged` | `T2.merged` |

某条 T2 route 不可用时，对应 T3 route 必须 `NOT_AVAILABLE` 并说明原因，不能静默借用另一条 route 的 T2 输出。

### 8.8 Merged 契约

T3 merged 对同一 L1 span identity 上的 code 和 AI decisions 做对账，输出完整 element decisions。merged 必须消费的不只是 policy，还包括 span identity、role、fill/generated/manual 语义、confidence、evidence 和 origin trace。

只写 observation artifact、只写 proposal、只 patch `candidate_policy` 或 accepted decision 没有进入 `element_spec`，都不算 T3 merged 消费完成。
...
| 关键不变量 | 源 package hash 与 L1 匹配；不重建第二套身份索引；每个动作有执行前置条件和 source/span/element trace；无内部 marker；执行结果可回查 |

## 12. T7 与 POST_T6：验证和最终差距

### T7

T7 读取 sealed L1、T5/T6 运行产物和最终 DOCX，汇总运行时结构验证，说明是否存在 schema、hash、引用、执行前置条件、动作结果或 coverage 问题。T7 可以用 L1 做 expected-vs-observed 对账并定位 first bad stage、root cause 和 owner，但不重新推断 T2/T3/T4 业务语义，也不读取学校 gold 来修改业务产物。
...
## 13. Code / AI / Merged Route 契约

T2/T3/T4 可同时存在三条 route：
