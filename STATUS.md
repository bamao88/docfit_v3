...
- real-core 模板合同现在还要求生成模板差距检查：`docfit eval template-gap`
  ...
  page/section 边界作为证据。
  最新三校 probe 写在 `/tmp/docfit_stage_realcore_audit32`，仍全部为 `FAIL`：
  湖南农业大学 PASS/FAIL/UNKNOWN 为 `161/6/127`，南农为 `94/20/92`，
  北大为 `109/13/31`。本轮主要修了源锚点和检查定位：南农正文不再跳到
  ...
  可见对象阻断；剩余未建模对象主要是文本框和图片。主要阻断仍集中在样式、页眉页脚、
  页码、未建模可见对象，以及页面/样式/生成机制类要求的证据不足。
