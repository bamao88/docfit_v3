...
- real-core 模板合同现在还要求生成模板差距检查：`docfit eval template-gap`
  ...
  最新三校 probe 写在 `/tmp/docfit_stage_realcore_audit15`，仍全部为 `FAIL`：
  湖南农业大学 PASS/FAIL/UNKNOWN 为 `145/21/128`，南农为 `79/26/100`，
  北大为 `86/32/34`。主要阻断已从元素缺失收敛到分页、页眉页脚、样式、
  单元顺序、字段绑定和页面/样式/生成机制类要求的证据不足。
